...
```typescript
// ✅ Next.js 16에서 필수: async request APIs만 지원
...
// ❌ 금지: 동기식 접근 (16에서 완전히 제거)
export default function Page({ params }: { params: { id: string } }) {
  const user = getUser(params.id) // ❌ 타입 에러 + 런타임 에러
  return <UserProfile user={user} />
}
```

### Typed Routes 활용 (선택사항)

```typescript
// ✅ Typed Routes로 타입 안전성 보장 (선택사항)
import Link from 'next/link'

// next.config.ts에서 활성화 (필요시)
...
export function Navigation() {
  return (
    <nav>
      ...
      {/* ❌ 컴파일 에러: 존재하지 않는 경로 (typedRoutes 활성화 시) */}
...
export async function updateProduct(id: string, data: ProductData) {
  ...
  // Next.js 16: revalidateTag는 두 번째 인자(cacheLife 프로필) 필수
  revalidateTag(`product-${id}`, 'max') // 또는 'auto', 'seconds=...' 등
  revalidateTag('products', 'max')
}
```

### Turbopack (기본값, 설정 선택사항)
...
// 특별한 설정이 필요 없으면 아래와 같이 최소 설정 유지
const nextConfig: NextConfig = {
  // Turbopack 커스텀 설정 (필요시)
  turbopack: {
    resolveAlias: {
      // 모듈 별칭 설정 (예: Node.js 모듈 폴백)
      // fs: { browser: './empty.ts' }
    },
  },
}
...
// ⚠️ 프로젝트에 필요시에만 추가
// middleware.ts는 deprecated → proxy.ts로 변경 권장
...
### unauthorized/forbidden (실험 단계)

```typescript
// app/admin/page.tsx
// ⚠️ authInterrupts 활성화 필요 (experimental)
// next.config.ts에 experimental.authInterrupts: true 설정 필요
