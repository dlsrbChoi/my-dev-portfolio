...
이 문서는 TailwindCSS v4 + shadcn/ui (`base-nova` 스타일)를 활용한 스타일링 규칙과 모범 사례를 제공합니다.

## 🎨 기술 스택 개요

### 핵심 스타일링 도구

- **TailwindCSS v4**: 유틸리티 기반 CSS 프레임워크 (CSS-first 설정)
- **shadcn/ui**: `@base-ui/react` 기반 컴포넌트 라이브러리 (base-nova 스타일)
- **next-themes**: 다크모드 지원 (라이트/다크 모드 자동 전환)
- **tw-animate-css**: 애니메이션 라이브러리
- **CSS Variables (oklch)**: 동적 테마 시스템 (oklch 색상 공간)
...
// ✅ shadcn/ui 컴포넌트 활용 (base-nova 스타일)
...
### CSS 변수 기반 색상 (oklch 형식)

`src/app/globals.css`에 정의된 색상 변수 (Tailwind v4 CSS-first):

```css
:root {
  --background: oklch(1 0 0);
  --foreground: oklch(0.235 0.022 257.36);
  --primary: oklch(0.265 0.053 260.34);
  --primary-foreground: oklch(0.98 0.007 261.77);
  --secondary: oklch(0.933 0.003 260);
  --secondary-foreground: oklch(0.265 0.053 260.34);
  --muted: oklch(0.933 0.003 260);
  --muted-foreground: oklch(0.55 0.015 256.67);
  --accent: oklch(0.933 0.003 260);
  --accent-foreground: oklch(0.265 0.053 260.34);
  --destructive: oklch(0.555 0.254 29.23);
  --destructive-foreground: oklch(0.98 0.007 261.77);
  --border: oklch(0.913 0.004 260);
  --input: oklch(0.913 0.004 260);
  --ring: oklch(0.235 0.022 257.36);
}

.dark {
  --background: oklch(0.133 0.004 260);
...
**⚠️ Tailwind v4 특징:**
- `@import "tailwindcss"` (CSS-first 설정)
- `@theme inline { ... }` 블록으로 CSS 변수를 Tailwind 유틸리티에 매핑
...
// ✅ 애니메이션 활성화 체크 (prefers-reduced-motion)
<div className="motion-safe:animate-bounce motion-reduce:transform-none">
  시스템 설정 존중
</div>
```
