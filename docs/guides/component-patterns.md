...
이 문서는 Next.js 16.2.12 + React 19.2.4 환경에서 효율적이고 재사용 가능한 컴포넌트 작성 패턴을 제공합니다.
...
### 3. Render Props 패턴 (고급)

```tsx
// ✅ Render Props 패턴 (고급 패턴)
...
```tsx
// ✅ 큰 리스트 처리: Suspense + Pagination 권장
// react-window 같은 가상화 라이브러리가 필요한 경우 npm install 후 사용
